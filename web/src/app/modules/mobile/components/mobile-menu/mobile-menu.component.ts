        ...
        this.categoryService.getAllCategorys().subscribe(data=>{
            data.forEach(item=>{
                ...
                item.subCategories.forEach(element => {
                    if(element.active){children.push({type: 'link',label:element.name,url:'/shop/catalog/'+item.id+'/'+element.id})}
                });
                if(children.length!==0){
                    let obj :any = {type: 'link',label:item.categoryName,children:children,url:'/shop/catalog/'+item.id};
