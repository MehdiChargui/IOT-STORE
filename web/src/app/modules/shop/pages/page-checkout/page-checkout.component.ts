...
export class PageCheckoutComponent implements OnInit, OnDestroy {
        ...
        this.cart.quantity$.pipe(takeUntil(this.destroy$)).subscribe(quantity => {
            if (!quantity) {
                ...
                this.cart.items$.subscribe(items=>{
                    this.order.details=[]
                    items.forEach(item=>{
                        let detail = new OrderDetail(item.product.id,item.quantity,item.priceTTC)
    ...
    getProducts(){
        this.articleService.getAllArticles().subscribe(data=>{
            ...
            this.cart.items.forEach(item=>{
                let productQty = data.find(pro=>pro.id===item.product.id).quantity;
                let newQty = productQty-item.quantity;
                if(newQty <= 2){this.alertBody.push(item.product.reference+":"+item.product.title)}
                this.array.push({article:item.product.id,qty:newQty})
            })
        })
    }
    getUser(id:string){
    ...
    submit(){
        ...
        if(this.alertBody.length!==0){this.sendAlert(this.alertBody.join(','))}
            this.cart.delete()
            this.router.navigate(['/shop/cart/checkout/success/'+data.id])
        }) 
    }

    sendMail(ref:any){
        let payment=this.paymentMethods.find(item=>item.id===this.order.paymentMethod).name;
        let delivery=this.deliveryMethods.find(item=>item.id===this.order.deliveryMethod).name
    ...
    updateQty(){
        console.log("update qte : "+this.array)
