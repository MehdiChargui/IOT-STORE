        ...
        this.categoryService.getAllCategorys().subscribe(data=>{
            ...
            data.forEach(item=>{
                ...
                item.subCategories.forEach(element => {
                    if(element.active){menu.items.push({label:element.name,url:'/shop/catalog/'+item.id+'/'+element.id})}
                });
                if(menu.items.length!==0){
                    let obj :any = {label:item.categoryName,menu:menu,url:'/shop/catalog/'+item.id};
                this.items.push(obj)
                }
